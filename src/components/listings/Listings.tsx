...
const Listings = () => {
	...
	const [params, setParams] = useState({
		...
		page: currentPage, // Add page to params
		limit: limit, // Add limit to params
	});
	...
	const {
		...
		func: listFunc,
		key: queryKey,
		params: { ...params, offset: (currentPage - 1) * limit, limit }, // Now includes page and limit
	});

	const [openBanDialog, setOpenBanDialog] = useState(false);
	const [selectedUser, setSelectedUser] = useState<TListing | null>(null);

	// Sync page parameter when currentPage changes
	useEffect(() => {
		setParams((prev) => ({
			...prev,
			page: currentPage,
	...
	const handleParamChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement> | SelectChangeEvent<string>) => {
		...
		setCurrentPage(1); // Also reset currentPage state
	};
	...
	const totalCount = Number(sanitizeValue(listings?.result?.count) ?? 0);
						...
						>
							...
							<CardActions sx={{ justifyContent: 'space-between', p: 2 }}>
								...
								{listing.is_active === 0 ? (
									...
									<Button
										onClick={() => handleOpenBanDialog(listing)}
										size="small"
										color="primary"
