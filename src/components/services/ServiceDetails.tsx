...
export default function ServiceDetails() {
	...
	const nextImage = () => {
		setCurrentImageIndex((prevIndex) => (prevIndex === serviceData?.images_list?.length - 1 ? 0 : prevIndex + 1));
	};
	const prevImage = () => {
		setCurrentImageIndex((prevIndex) => (prevIndex === 0 ? serviceData?.images_list?.length - 1 : prevIndex - 1));
	};
	...
	if (!serviceData) return <div>No rental data found</div>;
	return (
				...
				<div className="space-y-8">
					<div className="overflow-hidden bg-white rounded-lg shadow">
						<div className="relative aspect-video bg-muted">
							{serviceData.images_list?.length > 0 && (
								<>
									<img
										src={`${process.env.REACT_APP_GET_MEDIA}/${serviceData.images_list[currentImageIndex].image}`}
									...
									<div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2">
										{serviceData.images_list.map((_: any, index: number) => (
											<button
												key={index}
						...
						{serviceData.images_list?.length > 1 && (
							<div className="flex gap-2 p-2 overflow-x-auto">
								{serviceData.images_list.map((image: TRentalImage, index: number) => (
									<button
										key={index}
										className={`relative w-20 h-20 overflow-hidden rounded-md flex-shrink-0 ${
											index === currentImageIndex ? 'ring-2 ring-primary' : 'opacity-70'
										}`}
										onClick={() => setCurrentImageIndex(index)}
									>
										<img
											src={`${process.env.REACT_APP_GET_MEDIA}/${image.image}`}
					...
					<div className="bg-white rounded-lg shadow">
							...
							<div className="flex items-start justify-between">
								...
								<span
									className={`px-3 py-1 text-sm rounded-full ${
										serviceData.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
									}`}
								>
						...
						<div className="p-6 space-y-6">
							...
							<div>
								<h3 className="mb-4 text-lg font-semibold">Service Rates</h3>
								{serviceData.rates_list && serviceData.rates_list.length > 0 ? (
									<div className="overflow-hidden border border-gray-200 rounded-lg">
										<table className="min-w-full divide-y divide-gray-200">
											<thead className="bg-gray-50">
												<tr>
					...
					<div className="bg-white rounded-lg shadow">
						...
						<div className="p-6 space-y-4">
							<div className="flex items-center">
								...
								<span>{serviceData.seller_fullname || serviceData.seller_name}</span>
							</div>
							<div className="flex items-center">
								<Phone className="w-5 h-5 mr-2 text-muted-foreground" />
								<span>{serviceData.contact_phone}</span>
							</div>
						...
						<div className="p-6 space-y-4">
							<div className="flex items-start">
								...
								<div>
									...
									<p className="text-muted-foreground">{serviceData.delivery_time} days</p>
								</div>
							</div>
							<div className="flex items-start">
								...
								<div>
									...
									<p className="text-muted-foreground">
										{serviceData.delivery_fee ? `$${serviceData.delivery_fee}` : 'Contact seller for details'}
									</p>
								</div>
							</div>
						</div>
						...
						<div className="p-6 space-y-4">
							<div className="flex items-start">
								...
								<div>
									...
									<p className="text-muted-foreground">#{serviceData.id}</p>
								</div>
							</div>
							<div className="flex items-start">
								...
								<div>
									...
									<p className="text-muted-foreground">
										{serviceData.created_on ? new Date(serviceData.created_on).toLocaleDateString() : 'Not specified'}
									</p>
								</div>
							</div>
						</div>
