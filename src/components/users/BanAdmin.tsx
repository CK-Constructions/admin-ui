...
export function BanAdmin({ open, onClose, user, isBanned }: BanDialogProps) {
  const { mutationFn: banFunc, queryKey } = queryConfigs.useBanAdmin;
  const { mutationFn: unbanFunc, queryKey: unbanKey } = queryConfigs.useUnBanAdmin;

  const handleConfirmBan = () => {
    if (user.id) {
      banMutate({ id: user.id });
    } else {
      showNotification("error", "Support ID is Missing, Try Again");
    }
  };
  ...
  const { mutate: banMutate } = useMutationQuery({
    ...
    func: banFunc,
    onSuccess: () => {
      showNotification("success", "Support Banned Successfully");
      onClose();
    },
  });
  ...
  const { mutate: unbanMutate } = useMutationQuery({
    ...
    func: unbanFunc,
    onSuccess: () => {
      showNotification("success", "Support Unbanned Successfully");
      onClose();
    },
  });
