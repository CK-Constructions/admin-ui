...
import type { TRentalapproval } from "../lib/types/response";
...
export default function RentalApproval() {
  ...
  const [selectedUser, setSelectedUser] = useState<TRentalapproval | null>(null);
  const [selectedUserId, setSelectedUserId] = useState<number | null>(null);
  const [openDialog, setOpenDialog] = useState(false);
  const [selectedApproval, setSelectedApproval] = useState<TRentalapproval>({
    id: 0,
    approval_status: 0,
    rental_id: 0,
    category: 0,
  ...
  const handleOpenDialog = (approval: TRentalapproval) => {
    ...
    setSelectedApproval({
      ...
      rental_id: approval.rental_id || 0,
  ...
  const limit = 10;
  const { queryFn: updateServiceApproval, queryKeys: updateKey } = queryConfigs.useUpdateRentalApproval;
  ...
  const handleApprove = (approval: TRentalapproval) => {
    updateApproval({
      body: {
        rental_id: approval.rental_id,
        approval_status: 0,
      },
      id: approval.id,
    });
  };
  const handleReject = (approval: TRentalapproval) => {
    updateApproval({
      body: {
        rental_id: approval.rental_id,
        approval_status: 1,
  ...
  const { queryFn: rentalFunc, queryKeys: rentalKey } = queryConfigs.useActiveGetRental;
  ...
  const { queryFn: confirmedFunc, queryKeys: confirmedKey } = queryConfigs.useConfirmedGetRental;
  const {
    data: confirmedData,
  ...
  const { queryFn: rejectedFunc, queryKeys: rejectedKey } = queryConfigs.useRejectedGetRental;
  const {
    data: rejectedData,
  ...
  const handleOpenEdit = (rental: TRentalapproval) => {
    setEditingUserId(rental?.id);
    setOpenBanDialog(true);
  };
  const handleOpenBanDialog = (rental: TRentalapproval) => {
    setSelectedUser(rental);
    setOpenBanDialog(true);
  };
  const handleOpenViewDialog = (rental: TRentalapproval) => {
      ...
      <>
        <TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {currentTabData.data?.result.list.map((rental: TRentalapproval) => (
                <TableRow key={rental?.id}>
                  ...
                  <TableCell>
                    <section className="w-full flex gap-2">
                      <Tooltip title="View">
                        <button onClick={() => handleNavToView(rental.rental_id)} className="action-button">
                          <FaEye size={14} />
                        </button>
                      </Tooltip>
                      {tabValue === 0 && (
    ...
    <>
      <div className="pb-4">
        <Header
          ...
          showButton={false}
          buttonTitle="Add Rental"
          pageName="Vehicle Rental Approvals"
      ...
      <div className="flex flex-col h-full p-6">
        <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
          <Tabs value={tabValue} onChange={handleTabChange} aria-label="rental approval tabs" indicatorColor="primary" textColor="primary">
            <Tab label="Active Rentals" id="rental-tab-0" aria-controls="rental-tabpanel-0" />
            <Tab label="Rejected Rentals" id="rental-tab-1" aria-controls="rental-tabpanel-1" />
            <Tab label="Confirmed Rentals" id="rental-tab-2" aria-controls="rental-tabpanel-2" />
          </Tabs>
        </Box>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to {selectedApproval?.approval_status === 0 ? "approve" : "reject"} the approval for listing {selectedApproval?.rental_id}?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
