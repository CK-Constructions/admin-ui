...
import { UpdateActivityRentalCategory } from '../../api';
...
export default function RentalCategory() {
	...
	const [params, setParams] = useState<TQueryParams>({ id: '', name: '' });
	const [searchParams, setSearchParams] = useState<TQueryParams>({ id: '', name: '' });
	...
	const handleAddCategory = () => {
		...
		addRental({ name: trimmedName.toLowerCase() });
	};
	...
	const handleUpdateCategory = () => {
		...
		if (editingUserId) {
			updateRental({ id: editingUserId, body: { name: editCategoryName.toLowerCase() } });
		}
	};
		...
		<>
			...
			<div className="flex flex-col h-full p-6">
				...
				<div className="my-6 flex gap-2">
					<TextField
						...
						sx={{ width: '20%' }}
					/>
					<TextField name="id" label="ID" variant="outlined" size="small" value={params.id} onChange={handleSearchChange} sx={{ width: '20%' }} />
											...
											<TableCell sx={{ px: 2 }}>
												<section className="w-full flex gap-2">
													...
													{category.is_active === 0 && (
														<Tooltip title="Ban Category">
															<button onClick={() => setOpenBanDialog(true)} className="red-action-button">
																<FaBan size={14} />
															</button>
														</Tooltip>
													)}
													{category.is_active === 1 && (
														<Tooltip title="UnBan Category">
															<button onClick={() => setOpenBanDialog(true)} className="green-action-button">
			...
			<Dialog open={isModalOpen} onClose={handleCloseModal}>
				...
				<DialogActions>
					...
					<Button onClick={handleAddCategory} variant="contained" disabled={isAdding || !newCategoryName.trim()} sx={{ color: 'white' }}>
						{isAdding ? <CircularProgress size={24} /> : 'Add'}
					</Button>
				</DialogActions>
			</Dialog>

			{/* Edit Rental Category Modal */}
			<Dialog open={openEditDialog} onClose={handleCloseEditDialog}>
				...
				<DialogActions>
					...
					<Button onClick={handleUpdateCategory} variant="contained" sx={{ color: 'white' }} disabled={!editCategoryName.trim()}>
						Update
					</Button>
				</DialogActions>
			</Dialog>
